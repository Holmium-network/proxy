...
class ModuleDescriptor{constructor(id,heightPx,initializeCallback){this.id_=id;this.heightPx_=heightPx;this.element_=null;this.initializeCallback_=initializeCallback}get id(){return this.id_}get heightPx(){return this.heightPx_}get element(){return this.element_}async initialize(timeout){const loadStartTime=BrowserProxy.getInstance().now();this.element_=await Promise.race([this.initializeCallback_(),new Promise((resolve=>{BrowserProxy.getInstance().setTimeout((()=>{resolve(null)}),timeout)}))]);if(!this.element_){return}const loadEndTime=BrowserProxy.getInstance().now();BrowserProxy.getInstance().handler.onModuleLoaded(this.id_,loadEndTime,mojoTimeDelta(loadEndTime-loadStartTime))}}// Copyright 2020 The Chromium Authors. All rights reserved.
...
<!--_html_template_end_-->`}static get properties(){return{descriptor:{observer:"onDescriptorChange_",type:Object}}}onDescriptorChange_(newValue,oldValue){assert(!oldValue);this.$.moduleElement.appendChild(this.descriptor.element);this.$.moduleElement.style.height=`${this.descriptor.heightPx}px`;this.descriptor.element.addEventListener("usage",(()=>{BrowserProxy.getInstance().handler.onModuleUsage(this.descriptor.id)}),{once:true});const headerObserver=new IntersectionObserver((([{intersectionRatio:intersectionRatio}])=>{if(intersectionRatio>=1){headerObserver.disconnect();BrowserProxy.getInstance().handler.onModuleImpression(this.descriptor.id,BrowserProxy.getInstance().now())}}),{threshold:1});let intersectionPerdecage=0;const moduleObserver=new IntersectionObserver((([{intersectionRatio:intersectionRatio}])=>{intersectionPerdecage=Math.floor(Math.max(intersectionPerdecage,intersectionRatio*10))}),{threshold:[.1,.2,.3,.4,.5,.6,.7,.8,.9,1]});window.addEventListener("unload",(()=>{const recordPerdecage=(metricName,value)=>{chrome.metricsPrivate.recordValue({metricName:metricName,type:chrome.metricsPrivate.MetricTypeType.HISTOGRAM_LINEAR,min:1,max:11,buckets:12},value)};recordPerdecage("NewTabPage.Modules.ImpressionRatio",intersectionPerdecage);recordPerdecage(`NewTabPage.Modules.ImpressionRatio.${this.descriptor.id}`,intersectionPerdecage)}));microTask.run((()=>{headerObserver.observe(this.$.impressionProbe);moduleObserver.observe(this)}));this.addEventListener("mouseover",(()=>{chrome.metricsPrivate.recordSparseHashable("NewTabPage.Modules.Hover",this.descriptor.id)}),{useCapture:true,once:true})}}customElements.define(ModuleWrapperElement.is,ModuleWrapperElement);// Copyright 2020 The Chromium Authors. All rights reserved.
...
<!--_html_template_end_-->`}static get properties(){return{title:String,chipText:String,descriptionText:String,showInfoButton:{type:Boolean,value:false},showDismissButton:{type:Boolean,value:false},showMenuButton:{type:Boolean,value:false}}}onInfoButtonClick_(){this.dispatchEvent(new CustomEvent("info-button-click",{bubbles:true}))}onDismissButtonClick_(){this.dispatchEvent(new CustomEvent("dismiss-button-click",{bubbles:true}))}onMenuButtonClick_(){this.dispatchEvent(new CustomEvent("menu-button-click",{bubbles:true}))}}customElements.define(ModuleHeaderElement.is,ModuleHeaderElement);// Copyright 2020 The Chromium Authors. All rights reserved.
class ChromeCartProxy{constructor(){this.handler=chromeCart.mojom.CartHandler.getRemote()}}addSingletonGetter(ChromeCartProxy);// Copyright 2020 The Chromium Authors. All rights reserved.
class ChromeCartModuleElement extends PolymerElement{static get is(){return"ntp-chrome-cart-module"}static get template(){return html`<!--css-build:shadow--><!--_html_template_start_--><style include="cr-hidden-style cr-icons" scope="ntp-chrome-cart-module">:host {
  display: flex;
    flex-direction: column;
    height: 100%;
    width: 100%;
...
<ntp-module-header chip-text="[[headerChipText]]" description-text="[[headerDescriptionText]]" show-menu-button="" on-menu-button-click="onModuleMenuButtonClick_">
  Your carts
</ntp-module-header>
<div id="moduleContent">
      ...
      <a class="cart-item" title="[[item.merchant]]" href="[[item.cartUrl.url]]" on-click="onCartItemClick_" on-auxclick="onCartItemClick_">
        ...
        <div class="thumbnail-container">
          ...
          <template id="thumbnailFallback" is="dom-if" if="[[!item.productImageUrls.length]]">
            <img class="thumbnail-fallback" src="chrome://new-tab-page/icons/cart_fallback.svg">
          </template>
        </div>
      </a>
    </template>
  ...
  <cr-action-menu id="moduleActionMenu">
    <button id="hideModuleButton" class="dropdown-item" on-click="onModuleHide_">
      Hide these carts
    </button>
    <button id="removeModuleButton" class="dropdown-item" on-click="onModuleRemove_">
      Never show carts
    </button>
  </cr-action-menu>
...
<!-- TODO(crbug/1161005) Build Module UI --><!--_html_template_end_-->`}}customElements.define(DriveModuleElement.is,DriveModuleElement);async function createDriveElement(){const{testString:testString}=await DriveProxy.getInstance().handler.getTestString();console.log(testString);return new DriveModuleElement}const driveDescriptor=new ModuleDescriptor("drive",314,createDriveElement);// Copyright 2020 The Chromium Authors. All rights reserved.
class ModuleRegistry{constructor(){this.descriptors_=[]}registerModules(descriptors){this.descriptors_=descriptors}async initializeModules(timeout){await Promise.all(this.descriptors_.map((d=>d.initialize(timeout))));return this.descriptors_.filter((descriptor=>!!descriptor.element))}}addSingletonGetter(ModuleRegistry);// Copyright 2020 The Chromium Authors. All rights reserved.
...
<ntp-module-header show-info-button="" on-info-button-click="onInfoButtonClick_" show-dismiss-button="" on-dismiss-button-click="onDismissButtonClick_">
  [[task.title]]
</ntp-module-header>
<div id="moduleContent">
  <div id="taskItems">
...
<div id="content" style="--ntp-theme-text-color: [[rgbaOrInherit_(theme_.shortcutTextColor)]];
  ...
  <ntp-logo id="logo" doodle-allowed$="[[doodleAllowed_]]" single-colored$="[[singleColoredLogo_]]" dark="[[theme_.isDark]]" background-color="[[backgroundColor_]]" hidden$="[[!logoEnabled_]]">
  </ntp-logo>
